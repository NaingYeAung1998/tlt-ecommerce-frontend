...
import { calculateQuantityWithHierarchy, calculateQuantityWithProduct } from '@/app/utils';
...
const Tracks: FC<StockTrackProps> = ({ stock_id, product_id }) => {
    ...
    const getTracks = async () => {
        const unitHierarchy = await getProductUnitHierarchy();
        ...
        if (response.ok) {
            ...
            if (unitHierarchy) {
                result.data.map((track: any) => {
                    let formattedQuantity = calculateQuantityWithHierarchy(unitHierarchy, [{ unit_id: track.unit_id, unit_name: track.unit_name, quantity: track.quantity }])
                    track.quantity = formattedQuantity.quantityString;
                })
            }
            setRows(result.data);
    ...
    const getTrackInfo = async () => {
        let unitHierarchy = await getProductUnitHierarchy();
        const url = `${process.env.NEXT_PUBLIC_BACKEND_URL}stock/track/getInfo/${stock_id}`;
        let response = await fetch(url);
        if (response.ok) {
            let result = await response.json();
            if (unitHierarchy) {
                result.quantity = calculateQuantityWithHierarchy(unitHierarchy, [{ unit_id: result.unit_id, quantity: result.quantity }]).quantityString;
                result.total_available = calculateQuantityWithHierarchy(unitHierarchy, [{ unit_id: result.unit_id, quantity: result.total_available }]).quantityString
            }
            setTrackInfo(result);
        } else {
        ...
        <div>
            ...
            <Box sx={{ padding: 2 }}>
                <div className='sm:flex justify-between pb-5'>
                    ...
                    <div className='text-[16px]'><Typography color='#777777'>Quantity</Typography>  {trackInfo?.quantity ?? '0'} </div>
