...
const columns: readonly Column[] = [
    { id: 'supplier_name', label: 'Name', minWidth: 170 },
    { id: 'supplier_address', label: 'Address', minWidth: 100 },
    {
        id: 'supplier_phone',
        label: 'Phone',
        minWidth: 170,
...
export default function Suppliers() {
    ...
    const getSuppliers = async () => {
        const url = `${process.env.NEXT_PUBLIC_BACKEND_URL}supplier?search=${search}&currentPage=${currentPage}&perPage=${perPage}`;
        let response = await fetch(url);
        if (response.ok) {
    ...
    return (
        <div>
            ...
            <Paper sx={{ width: '100%', overflow: 'hidden' }}>
                <Alert severity='success' hidden={!showSuccess}>{`Supplier ${supplier} successfully ${action === 'update' ? 'updated' : 'created'}.`}</Alert>
                <Box sx={{ padding: 2 }}>
                    <div className='flex justify-end pb-5'>
                        <Link href={'/dashboard/supplier/create'}>
                            <Button variant='contained' color='primary'>Add Supplier</Button>
                        </Link>
                    </div>
                    <Grid container sx={{ paddingTop: '20px' }}>
                        <Grid size={6}>
                            <Typography variant='body1' fontWeight={'bold'}>Suppliers</Typography>
                        </Grid>
                        <Grid size={6}>
                    ...
                    <Table aria-label="sticky table" stickyHeader={true}>
                        ...
                        <TableBody>
                            {rows
                                .map((row, index) => {
                                    return (
                                        <TableRow hover role="checkbox" tabIndex={-1} key={row.supplier_id}>
